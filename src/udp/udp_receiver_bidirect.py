...
def receive_and_echo(listen_port, response_ip, response_port, listen_ip=None, iface=None):
    """Listen for incoming UDP packets and echo the sequence number back."""
    ...
    try:
        while True:
            try:
                ...
                try:
                    seq = int(message)
                    # Echo the sequence number back to the sender
                    response = str(seq)
                    sock.sendto(response.encode('utf-8'), (response_ip, response_port))
                    echoed_count += 1
                    print(f"[ECHO #{seq}] from {addr} -> to {response_ip}:{response_port}")
                except ValueError:
                    print(f"[WARNING] Received non-numeric payload from {addr}: {message}")
...
def main():
    parser = argparse.ArgumentParser(description="UDP receiver that echoes sequence numbers back to sender.")
